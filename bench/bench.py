...
gpu_filters_bin = working_dir.parent / "target" / "release" / "gpu_filters.exe"
max_workers = 16
seed = 1337


def get_inference_image():
    # This should be synced with how gpu_filters generates its image (2048 squared, 4 channels, random int)
    shape = (2048, 2048, 4)
    return numpy.random.randint(0, 255, shape)
...
def create_training_data():
    ...
    iio.imwrite(str(working_dir / "randint2048x2048x4.tif"), img)
...
def run_on_cpu(classifier):
    ...
    start = perf_counter()
    img = get_inference_image()
    features = get_features(img)
    prediction = classifier.predict(features)
    iio.imwrite(str(cpu_seg_output_path), prediction.reshape(img.shape[0], img.shape[1]))
    end = perf_counter()
